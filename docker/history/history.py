...
stm = DPay(node=["https://" + os.environ['dpaynode']], custom_chains={"DPAY":
...
db = mongo.dpayexplorer

mvest_per_account = {}

def load_accounts():
    pprint("DPayExplorer - Loading mvest per account")
...
def update_history():
    ...
    pprint("DPayExplorer - Update Global Properties")
    ...
    pprint("DPayExplorer - Update History (" + str(len(users)) + " accounts)")
    # Snapshot User Count
